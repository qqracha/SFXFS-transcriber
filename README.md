...
- отдельный режим импорта готовых `JSON`, `Markdown` и `TXT` транскрипций;
- сохранение таймкодов, дополнительных полей сегментов и проектов между перезапусками;
- те же 25-минутные LOGGER-чанки с overlap 5 минут без повторного запуска Whisper.
...
На стартовом экране выберите «Импорт транскрипции» и добавьте `JSON`, `MD` или `TXT`. JSON с `segments` предпочтителен; Markdown/TXT понимают строки `[00:00:23] текст`, `[01:15:42.350] текст` и `00:00:23 --> 00:00:31`. Если таймкодов нет, приложение разрешает смысловой анализ, но предупреждает, что CUTTER и DaVinci-маркеры будут ненадёжны. Проект сохраняется в `projects/<stream_id>` и доступен в списке последних проектов.
